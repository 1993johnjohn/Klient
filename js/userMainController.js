...
for (let i = 0 ; i < orders.length ; i++) {
    ...
    isReady= () => {
        if(order.isReady === true) {
            return "class= 'ready' > your order is ready for pickup"
        } else {
            return "class= 'notReady' >  Your order is not ready for pickup"
        }
    };
    ...
    $(`<h4>Total price:  <span style="position: absolute; right: 12%;">${total} kr.</span></h4>`).insertBefore('*[data-id='+ order.orderId +'] table')
}
});
});
